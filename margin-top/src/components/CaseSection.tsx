  ...
  return (
    <div className="py-24 bg-white text-black">
      <section className="max-w-6xl mx-auto text-center mb-12">
        <h2 className="text-4xl font-bold mb-4 md:mb-8 text-left">Recent Werk</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {caseData.slice(0, 3).map((caseItem) => ( // Show only the first 3 cases
            <div key={caseItem.id} className="relative border border-gray-200 rounded-lg overflow-hidden transition-transform transform hover:scale-105 cursor-pointer">
              <Image 
                src={caseItem.image} 
              ...
              <div className="p-6 text-left flex flex-col justify-between" style={{ height: '250px' }}>
                ...
                <div className="text-right mt-auto"> {/* Pushes the link to the bottom right */}
                  <Link href={`/cases/${caseItem.id}`} className="text-lg hover:underline" style={{ color: '#340066' }}>
