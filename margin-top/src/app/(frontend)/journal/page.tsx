...
async function JournalPage() {
  ...
  // Get all published journal posts
  const response = await fetch(`${protocol}://${host}/api/journal`, {
    next: { revalidate: 0 }
  });

  if (!response.ok) {
    console.error('Failed to fetch journal posts:', await response.text());
    return (
      <div className="bg-white">
        <section className="w-11/12 max-w-5xl mx-auto py-12 md:py-16 mt-4 md:mt-8">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mb-8">
            Er zijn momenteel geen journal posts beschikbaar.
          </p>
        </section>
      </div>
    );
  ...
  const posts = data.docs as Journal[];
